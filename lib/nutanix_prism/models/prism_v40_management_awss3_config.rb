...
module NutanixPrism
  ...
  class PrismV40ManagementAWSS3Config
    ...
    def self.openapi_types
      {
        :'bucket_name' => :'String',
        :'region' => :'String',
        :'credentials' => :'PrismV40ManagementAccessKeyCredentials'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `NutanixPrism::PrismV40ManagementAWSS3Config` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `NutanixPrism::PrismV40ManagementAWSS3Config`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if !@region.nil? && @region.to_s.length > 63
        invalid_properties.push('invalid value for "region", the character length must be smaller than or equal to 63.')
      end
    ...
    def valid?
      ...
      return false if !@region.nil? && @region.to_s.length > 63
      true
    end
    ...
    def region=(region)
      ...
      if region.to_s.length > 63
        fail ArgumentError, 'invalid value for "region", the character length must be smaller than or equal to 63.'
      end
    ...
    def hash
      [bucket_name, region, credentials].hash
    end
